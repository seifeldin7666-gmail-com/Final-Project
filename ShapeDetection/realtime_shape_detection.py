...
cv2.createTrackbar("Threshold 1", "Parameters", 70, 255, nothing) # value of threshold 1
cv2.createTrackbar("Threshold 2", "Parameters", 74, 255, nothing) # value of threshold 2
cv2.createTrackbar("Minimum Area", "Parameters", 9000, 50000, nothing)
...
while True:
    _, frame = cap.read()
    #frame = cv2.imread("Shape Detection.png")
    ...
    kernel = np.ones((5, 5))
    dilateion = cv2.dilate(canny, kernel) # dilate to get rid of some noise
    get_contours(dilateion, copied_frame)
