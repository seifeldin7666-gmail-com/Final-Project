...
void IMU_begin(){
  ...
  while (c < 200) {
    ...
    AccX = (Wire.read() << 8 | Wire.read()) / GR_CONST ;
    AccY = (Wire.read() << 8 | Wire.read()) / GR_CONST ;
    AccZ = (Wire.read() << 8 | Wire.read()) / GR_CONST ;
    // Sum all readings
...
//   Wire.read()<<8 | Wire.read(); // reading registers: 0x43 (GYRO_XOUT_H) and 0x44 (GYRO_XOUT_L)
//   Wire.read()<<8 | Wire.read(); // reading registers: 0x45 (GYRO_YOUT_H) and 0x46 (GYRO_YOUT_L)
//   Wire.read()<<8 | Wire.read(); // reading registers: 0x47 (GYRO_ZOUT_H) and 0x48 (GYRO_ZOUT_L)
// }
...
void IMU_readValues(){
  ...
  yaw =   yaw  + (abs(GyroZ) <= 3.00 ? 0 :GyroZ ) *elapsedTime;
...
float IMU_getValue(char type ){
    switch(type){
        case GET_X:
            return (float)(AccX/GR_CONST)*9.81;
        case GET_Y:
            return (float)(AccY/GR_CONST)*9.81;
        case GET_Z:
            return (float) (AccZ/GR_CONST)*9.81 ;
//        case GET_TEMP:
